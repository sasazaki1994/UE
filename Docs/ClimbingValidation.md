...
同じPRブランチへ先行して入っていたPR #8（`ecbea0e`）の日本語受け入れ仕様も取り込みました。READMEは統合後の動作を記載し、旧戦闘版の説明は `Docs/OriginalCombatPrototype.md` に保持しています。この追加統合は文書のみで、検証後にC++・入力・アセットを変更していません。
