...
void ACampaignHUD::DrawHUD()
{
    ...
    {
        const TCHAR* T[] = {TEXT("白面の祓い手。"), TEXT("境界石は砕かれ、禍が流れ出した。"), TEXT("禍は祓い手の左腕にも宿っている。"),
            TEXT("主に宿った禍を、境断ちで鎮めよ。")};
    ...
    {
        const TCHAR* T[] = {TEXT("石走りは静まった。"), TEXT("水の底でも、同じ脈動が続いている。"), TEXT("第二の主　淵纏い")};
    ...
    {
        const TCHAR* T[] = {TEXT("山の奥へ進むほど、左腕の痛みは強くなる。"), TEXT("第三の主　峰抱き")};
        Heading = TEXT("INTERLUDE");
    ...
    {
        const TCHAR* T[] = {TEXT("三柱を鎮めても、禍は消えなかった。"), TEXT("流れはすべて、一つの場所へ向かっている。"), TEXT("禍津根")};
        Heading = TEXT("INTERLUDE");
        Body = T[FMath::Clamp(I, 0, 2)];
        break;
    }
    ...
    {
        const TCHAR* T[] = {TEXT("禍津根は破壊されず、ただ鎮まった。"), TEXT("主たちは生きている。"),
            TEXT("左腕の禍も、まだ完全には消えていない。"), TEXT("禍は自然とともに、再び生じるかもしれない。")};
        Heading = TEXT("ENDING");
        Body = T[FMath::Clamp(I, 0, 3)];
        break;
    }
