...
void APrototypeHUD::DrawHUD()
{
    ...
    if (Player && Mode->IsEncounterActive() && PlayerOwner)
    {
    ...
    float Y = 18.f * Scale;
    const float Width = FMath::Min(620.f * Scale, Canvas->ClipX - X * 2.f);
    ...
    Line(TEXT("WASD move | Mouse aim | LMB slash along cyan arrow"));
