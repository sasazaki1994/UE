...
void AMagatsunePlayer::ConfigureBoss(AMagatsuneBoss* V) { Boss=V; AddTickPrerequisiteActor(Boss); Grab->AddTickPrerequisiteActor(this); SetPrimitiveColor(FindComponentByClass<UStaticMeshComponent>()->CreateDynamicMaterialInstance(0),FLinearColor(1,.66,.12)); }
...
void AMagatsunePlayer::JumpPressed(){if(IsMounted())Fall();else Jump();} void AMagatsunePlayer::PurifyPressed(){if(Boss)Boss->TryPurify();} void AMagatsunePlayer::RetryPressed(){if(auto* M=GetWorld()->GetAuthGameMode<AMagatsuneGameMode>())M->RetryEncounter();}
