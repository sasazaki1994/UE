...
{
    PrimaryActorTick.bCanEverTick=false; SetRootComponent(CreateDefaultSubobject<USceneComponent>(TEXT("ArenaRoot")));
    ...
    auto Part=[&](const TCHAR* Name,FVector Position,FVector Scale)
    {
        ...
        Mesh->SetStaticMesh(Cube.Object); Mesh->SetMaterial(0,Material.Object);
        Mesh->SetRelativeLocation(Position); Mesh->SetRelativeScale3D(Scale); Mesh->SetCollisionProfileName(TEXT("BlockAll"));
    };
...
void AMinedakiArena::BeginPlay()
{
    ...
    for(auto* Part:Parts) SetPrimitiveColor(Part->CreateDynamicMaterialInstance(0),Part->GetName().Contains(TEXT("HandHold"))?FLinearColor(.8,.6,.25):Part->GetName().Contains(TEXT("HeightBand"))?FLinearColor(.65,.62,.48):FLinearColor(.22,.3,.26));
    ...
    Fill->GetLightComponent()->SetMobility(EComponentMobility::Movable); Fill->GetLightComponent()->SetIntensity(2); Fill->GetLightComponent()->SetCastShadows(false);
