...
void AMinedakiHUD::DrawHUD()
{
    ...
    {
        DrawText(Text, Color, 24, Y, GEngine->GetMediumFont());
        Y += 25;
    };
    Line(TEXT("MINEDAKI / THE CLIMBING MOUNTAIN"), FLinearColor(.4, 1, .8));
    Line(FString::Printf(TEXT("State: %s | KAKON %d/3 | STAMINA %.0f/100 | NODE %d/%d"), *B->GetActionLabel(),
        B->GetNushiProgressComponent()->GetPurifiedCount(), P->GetStamina()->GetCurrentStamina(), P->GetRouteNode() + 1,
        AMinedakiBoss::RouteNodeCount));
    ...
    Line(Victory                       ? TEXT("VICTORY - MINEDAKI CALMED / ENCOUNTER COMPLETED | R / Y: Retry")
            ...
            ? TEXT("FINAL HEAD ROUTE OPEN. Rest at NODE 12, then KAKON 3 at NODE 14")
    ...
    Line(FString::Printf(TEXT("NEXT ROUTE <= NODE %d | HEIGHT %.1fm | Cling %.1fs | Shake %d | Falls %d | Recovery %d"),
        B->GetMaximumRouteNode() + 1, P->GetActorLocation().Z / 100, B->Telemetry.ClingSeconds, B->Telemetry.Shakes, B->Telemetry.Falls,
        B->Telemetry.RecoverySuccesses));
}
