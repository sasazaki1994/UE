...
void AMinedakiIntegrationTest::Hold(FKey Key,bool Down)
{
    ...
    FKey Mapped=Key;
    if(bGamepad) { if(Key==EKeys::E) Mapped=EKeys::Gamepad_RightShoulder; if(Key==EKeys::R) Mapped=EKeys::Gamepad_FaceButton_Top; if(Key==EKeys::SpaceBar) Mapped=EKeys::Gamepad_FaceButton_Bottom; if(Key==EKeys::LeftMouseButton) Mapped=EKeys::Gamepad_FaceButton_Left; }
...
void AMinedakiIntegrationTest::Tick(float Dt)
{
    ...
    auto* P=Mode->GetPlayer(); auto* B=Mode->GetBoss(); auto* PC=Cast<APlayerController>(P->GetController());
    if(bGamepad) { PC->InputKey(FInputKeyEventArgs::CreateSimulated(EKeys::Gamepad_LeftY,IE_Axis,ForwardAxis)); PC->InputKey(FInputKeyEventArgs::CreateSimulated(EKeys::Gamepad_LeftX,IE_Axis,RightAxis)); }
    Time+=Dt; Total+=Dt; if(Total>300||Time>50) { Check(false,TEXT("Input playthrough timeout")); return; }
    if(P->IsMounted())
    {
    ...
    PeakPitch=FMath::Max(PeakPitch,static_cast<float>(B->GetBodyRoot()->GetRelativeRotation().Pitch)); PeakYaw=FMath::Max(PeakYaw,FMath::Abs(static_cast<float>(B->GetBodyRoot()->GetRelativeRotation().Yaw))); PeakRoll=FMath::Max(PeakRoll,FMath::Abs(static_cast<float>(B->GetBodyRoot()->GetRelativeRotation().Roll)));
    switch(Phase)
    {
    case 0: if(Time>.1f) Shot(TEXT("01-GroundGrab")); MoveToward(B->GetRouteWorld(0)); if(FVector::Dist(P->GetActorLocation(),B->GetRouteWorld(0))<P->GrabRange-30) { Move(0); Tap(EKeys::E); Next(1); } break;
    case 1: if(Time>.2f) { if(!Check(P->IsMounted()&&P->GetRouteNode()==0,TEXT("Ground Grab through shared component"))) return; Move(1); Next(2); } break;
    case 2: if(B->IsWallMoving()) { Move(0); Hold(EKeys::E,true); WallRelative=P->GetGrab()->GetRelativeGrabTransform(); Shot(TEXT("02-Phase1WallClimb")); Next(3); } break;
    case 3:
        if(!P->IsMounted()) { Check(false,TEXT("Phase1 cling failed")); return; }
        if(B->GetActionState()==EMinedakiActionState::Shaking) { bSawShake=true; Shot(TEXT("03-FirstShake")); }
        if(B->GetActionState()==EMinedakiActionState::UpperPlatform) { Hold(EKeys::E,false); Move(1); Next(4); }
        break;
    case 4: if(P->GetRouteNode()==AMinedakiBoss::Kakon1Node&&!P->IsRouteMoving()) { Move(0); Shot(TEXT("04-Kakon1")); Tap(EKeys::LeftMouseButton); Hold(EKeys::E,true); Next(5); } break;
    case 5:
