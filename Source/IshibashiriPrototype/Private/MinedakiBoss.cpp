...
AMinedakiBoss::AMinedakiBoss()
{
    ...
    Part(TEXT("Pelvis"), {0, 0, 720}, {4, 5, 4});
    Part(TEXT("Torso"), {0, 0, 1100}, {5, 6.5, 8});
    Part(TEXT("Head"), {-40, 0, 1640}, {3.5, 3.8, 3.8});
    Part(TEXT("Muzzle"), {-205, 0, 1580}, {1.5, 2.7, 1.7});
    LeftArm = Part(TEXT("LeftArm"), {-70, -390, 1020}, {2.2, 2.2, 9});
    RightArm = Part(TEXT("RightArm"), {-70, 390, 1020}, {2.2, 2.2, 9});
    Part(TEXT("LeftLeg"), {0, -205, 350}, {2.5, 2.5, 7});
    Part(TEXT("RightLeg"), {0, 205, 350}, {2.5, 2.5, 7});
    for (int32 I = 0; I < RouteNodeCount; ++I)
        RouteMarkers.Add(Part(*FString::Printf(TEXT("Route%d"), I), GetRouteLocal(I), {.45, .45, .45}));
    for (int32 I = 0; I < 3; ++I)
        CoreMarkers.Add(Part(*FString::Printf(TEXT("KakonMarker%d"), I),
            GetRouteLocal(I == 0 ? Kakon1Node
                    : I == 1     ? Kakon2Node
                                 : Kakon3Node) +
                FVector(-55, 0, 0),
            {1.2, 1.2, 1.2}));
...
FVector AMinedakiBoss::GetRouteLocal(int32 Node) const
{
    ...
    static const FVector Nodes[] = {{185, -205, 180}, {205, -205, 450}, {255, -150, 740}, {310, -80, 970}, {325, 0, 1190},
        {280, -175, 1380}, {230, -90, 1500}, {200, 0, 1660}, {120, -330, 1460}, {-40, -570, 1510}, {-210, -760, 1570}, {120, 250, 1490},
        {40, 180, 1690}, {-30, 40, 1860}};
...
{
    Super::BeginPlay();
    SpawnPose = GetActorTransform();
    GrabFrame = GetWorld()->SpawnActor<AActor>();
    auto* FrameRoot = NewObject<USceneComponent>(GrabFrame, TEXT("BodyGrabFrame"));
    GrabFrame->SetRootComponent(FrameRoot);
    FrameRoot->RegisterComponent();
    GrabFrame->AttachToComponent(BodyRoot, FAttachmentTransformRules::SnapToTargetNotIncludingScale);
    GrabFrame->SetOwner(this);
    for (int32 I = 0; I < 3; ++I)
    {
        auto* Core = GetWorld()->SpawnActor<AKakonActor>();
        Core->SetOwner(this);
        Core->AttachToComponent(BodyRoot, FAttachmentTransformRules::SnapToTargetNotIncludingScale);
        Core->SetActorRelativeLocation(GetRouteLocal(I == 0 ? Kakon1Node : I == 1 ? Kakon2Node : Kakon3Node));
        Core->SetActorEnableCollision(false);
        Core->OnPurified.AddUniqueDynamic(this, &AMinedakiBoss::HandleKakonPurified);
        Kakons.Add(Core);
        RegisterKakon(Core);
    }
    ...
    for (UStaticMeshComponent* Marker : RouteMarkers)
        SetPrimitiveColor(Cast<UMaterialInstanceDynamic>(Marker->GetMaterial(0)), FLinearColor(.1, 1, .65));
    for (UStaticMeshComponent* Marker : CoreMarkers)
        SetPrimitiveColor(Cast<UMaterialInstanceDynamic>(Marker->GetMaterial(0)), FLinearColor(1, .07, .12));
    ResetNushi();
}
...
void AMinedakiBoss::EndPlay(const EEndPlayReason::Type Reason)
{
    ...
    for (AKakonActor* Core : Kakons)
        if (IsValid(Core)) Core->Destroy();
...
void AMinedakiBoss::ResetNushi()
{
    ...
    RightArm->SetRelativeRotation(FRotator::ZeroRotator);
    LeftArm->SetRelativeScale3D({2.2, 2.2, 9});
    RightArm->SetRelativeScale3D({2.2, 2.2, 9});
    for (UStaticMeshComponent* Marker : CoreMarkers)
    {
    ...
    for (AKakonActor* Core : Kakons)
        if (Core) Core->SetActorHiddenInGame(true);
    SetRouteVisibility();
}
...
{ return ActionState == EMinedakiActionState::ArmBridgeTransition || ActionState == EMinedakiActionState::FinalTransition; }
...
int32 AMinedakiBoss::GetRecoveryNode() const
{
    const int32 C = GetNushiProgressComponent()->GetPurifiedCount();
    return C >= 2 ? 11 : C >= 1 ? 7 : 0;
}
...
FString AMinedakiBoss::GetActionLabel() const
{ return StaticEnum<EMinedakiActionState>()->GetNameStringByValue(static_cast<int64>(ActionState)); }
...
void AMinedakiBoss::NotifyRouteNode(int32 Node)
{
    ...
    {
        ActionState = EMinedakiActionState::PreparingClimb;
        ++Telemetry.WallStarts;
        ++Telemetry.Phase1Starts;
        LogTelemetry(TEXT("Phase1Started"));
    }
}
...
{
    if (!FMath::IsFinite(Dt) || Dt <= 0 || PrepareSeconds <= 0 || WallSeconds <= 0 || LedgeSeconds <= 0 ||
        GetNushiState() != ENushiState::Active || !IsWallMoving())
        return;
    ...
    {
        float T = (ClimbTime - PrepareSeconds) / WallSeconds, Lift = Smooth(FMath::Fmod(T * 2, 1.f));
    ...
    else
    {
        ...
        Rotation = {MaximumPitch * (1 - T), MaximumYaw * (1 - T), 0};
    ...
    if (!bShakeApplied && ClimbTime >= PrepareSeconds + WallSeconds * .5f)
    {
        ...
        LogTelemetry(TEXT("Phase1Shake"));
    }
    if (ClimbTime >= Total)
    {
        ActionState = EMinedakiActionState::UpperPlatform;
        ++Telemetry.UpperReached;
        ++Telemetry.Phase1Completes;
        LeftArm->SetRelativeRotation(FRotator::ZeroRotator);
        RightArm->SetRelativeRotation(FRotator::ZeroRotator);
        LeftArm->SetRelativeLocation({-70, -390, 1020});
        RightArm->SetRelativeLocation({-70, 390, 1020});
        LeftArm->SetRelativeScale3D({2.2, 2.2, 9});
        RightArm->SetRelativeScale3D({2.2, 2.2, 9});
        GetKakon(0)->SetActorHiddenInGame(false);
        GetKakon(0)->ApplyShellDamage(GetKakon(0)->MaxShellHealth);
        CoreMarkers[0]->SetVisibility(true);
        SetRouteVisibility();
        LogTelemetry(TEXT("Phase1Completed"));
    }
}
...
void AMinedakiBoss::AdvancePostKakon(float Dt)
{
    ...
    if (IsBodyTransitioning())
    {
        ...
        if (ActionState == EMinedakiActionState::ArmBridgeTransition)
        {
            ...
            LeftArm->SetRelativeLocation(FMath::Lerp(FVector(-70, -390, 1020), FVector(-30, -610, 1450), T));
            LeftArm->SetRelativeRotation({0, 0, 70 * T});
            LeftArm->SetRelativeScale3D({2.2, 2.2, FMath::Lerp(9.f, 12.f, T)});
        }
        else
        {
            BodyRoot->SetRelativeRotation({FMath::Lerp(-12.f, 28.f, T), FMath::Lerp(18.f, -16.f, T), FMath::Lerp(-20.f, 30.f, T)});
    ...
    if (ActionState == EMinedakiActionState::Calming)
    {
        if (CalmSeconds <= 0)
        {
            BodyRoot->SetRelativeRotation(FRotator(8, 0, 0));
            ActionState = EMinedakiActionState::Calm;
            LogTelemetry(TEXT("Calm"));
            return;
        }
        CalmTime = FMath::Min(CalmTime + Dt, CalmSeconds);
        const float T = FMath::SmoothStep(0.f, 1.f, CalmTime / CalmSeconds);
        BodyRoot->SetRelativeRotation(FMath::Lerp(FRotator(28, -16, 30), FRotator(8, 0, 0), T));
...
void AMinedakiBoss::BeginPhase(int32 Phase)
{
    ...
    if (Phase == 2)
    {
        ActionState = EMinedakiActionState::ArmBridgeTransition;
        ++Telemetry.Phase2Starts;
        LogTelemetry(TEXT("Phase2Started_BodyRouteTransition"));
    }
    else
    {
        ActionState = EMinedakiActionState::FinalTransition;
        ++Telemetry.Phase3Starts;
        LogTelemetry(TEXT("Phase3Started_BodyRouteTransition"));
    }
    SetRouteVisibility();
}
...
void AMinedakiBoss::FinishTransition()
{
    ...
    Kakon->SetActorHiddenInGame(false);
    Kakon->ApplyShellDamage(Kakon->MaxShellHealth);
    CoreMarkers[KakonIndex]->SetVisibility(true);
    if (bArmTransition)
    {
        ++Telemetry.Phase2Completes;
        LogTelemetry(TEXT("Phase2Completed_ArmRouteOpen"));
    }
    else
    {
        ++Telemetry.Phase3Completes;
        LogTelemetry(TEXT("Phase3Completed_FinalRouteOpen"));
    }
    SetRouteVisibility();
}
...
void AMinedakiBoss::HandleKakonPurified(AKakonActor* Kakon)
{
    ...
    {
        Telemetry.CompletionSeconds = Telemetry.Elapsed;
        ActionState = EMinedakiActionState::Calming;
        CalmTime = 0;
        LogTelemetry(TEXT("Kakon3_EncounterCompleted_Victory"));
    }
...
bool AMinedakiBoss::TryPurifyKakon()
{
    ...
    const int32 Index = GetNushiProgressComponent()->GetPurifiedCount();
    const int32 Required[] = {Kakon1Node, Kakon2Node, Kakon3Node};
    return Index < 3 && Player->GetRouteNode() == Required[Index] &&
        FVector::Dist(Player->GetActorLocation(), GetRouteWorld(Required[Index])) <= 110 && GetKakon(Index)->Purify();
}
...
void AMinedakiBoss::SetRouteVisibility()
{
    ...
    const int32 Max = GetMaximumRouteNode();
    for (int32 I = 0; I < RouteMarkers.Num(); ++I)
        RouteMarkers[I]->SetVisibility(Reveal && I <= Max && !(IsBodyTransitioning() && I > Max));
    const int32 Active = GetNushiProgressComponent()->GetPurifiedCount();
    for (int32 I = 0; I < CoreMarkers.Num(); ++I)
        CoreMarkers[I]->SetVisibility(Reveal && I == Active && GetKakon(I) && !GetKakon(I)->IsHidden());
}

void AMinedakiBoss::UpdateArmHolds()
{
    const float WallT = FMath::Clamp((ClimbTime - PrepareSeconds) / WallSeconds, 0.f, 1.f),
                PrepareT = FMath::Clamp(ClimbTime / PrepareSeconds, 0.f, 1.f),
                LedgeT = FMath::Clamp((ClimbTime - PrepareSeconds - WallSeconds) / LedgeSeconds, 0.f, 1.f);
    for (int32 Side = 0; Side < 2; ++Side)
    {
        ...
        const FVector Hold = SpawnPose.TransformPosition(
            FVector(-2040, Sign * 480, FMath::Lerp(Side == 0 ? 600.f : 1020.f, Side == 0 ? 1440.f : 1860.f, Reach)));
        ...
        Arm->SetRelativeScale3D({1.9, 1.9, Axis.Size() / 100.f});
    }
}
...
void AMinedakiBoss::LogTelemetry(const TCHAR* Event) const
{
    UE_LOG(LogTemp, Display,
        TEXT(
            "MINEDAKI_TELEMETRY %s grab=%d/%d phase=%d/%d/%d kakon=%d route_transitions=%d cling=%.2f shakes=%d success=%d failure=%d falls=%d recovery=%d/%d exhaustion=%d elapsed=%.2f clear=%.2f"),
        Event, Telemetry.GrabSuccesses, Telemetry.GrabAttempts, Telemetry.Phase1Completes, Telemetry.Phase2Completes,
        Telemetry.Phase3Completes, Telemetry.KakonPurified, Telemetry.BodyRouteTransitions, Telemetry.ClingSeconds, Telemetry.Shakes,
        Telemetry.ShakeSuccesses, Telemetry.ShakeFailures, Telemetry.Falls, Telemetry.RecoverySuccesses, Telemetry.RecoveryStarts,
        Telemetry.Exhaustions, Telemetry.Elapsed, Telemetry.CompletionSeconds);
}
