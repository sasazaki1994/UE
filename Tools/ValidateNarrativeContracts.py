...
def audit_contract(contract: dict) -> list[str]:
    issues = []
    expected = {
        "schema_version": 1, "status": "DESIGN_ONLY",
    ...
    if not isinstance(delivery, dict) or delivery.get("appearance_runtime") != "NOT_IMPLEMENTED" or delivery.get("ue_runtime_validation") != "NOT_RUN":
        issues.append("CONTRACT: do not label the design-only delivery as runtime validated")
    return issues
...
def main() -> int:
    ...
    print("UE_RUNTIME=NOT_RUN; APPEARANCE_RUNTIME=NOT_IMPLEMENTED; no gameplay certification")
