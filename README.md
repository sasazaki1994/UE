...
> **制作優先順位（2026-09-24）:** 最初に見せられる体験は石走り編（接近路→石走り→短い結末）へ絞ります。作業範囲、250〜500時間の仮見積もり、UE実機ゲートは [石走り編を最初の完成目標にする](Docs/IshibashiriDemoFirstPlan.md) を参照してください。4戦Campaignは後続の本編基盤であり、石走り編の専用Endingや実機検証はまだ完了していません。
...
石走り戦は **突進を横回避 → Recover中に脚・角の禍の隆起を斬って体勢値を3回崩す → 5秒間の膝つき中に前脚または角から取り付く → 11地点Routeを登る → 背中の3禍根だけを祓う** 流れです。地上斬撃は石走り本体へダメージを与えず、体勢値0も勝利条件ではありません。登録された3禍根の浄化だけが `OnAllPurified → CalmNushi → Encounter Completed → Victory` へ進み、石走りは死亡・消滅せず鎮まります。
...
| ジャンプ | Space | A |
| 取り付く | 体勢を崩し、膝つき中に前脚・角付近の金色の印でE | 同じ場所でRB |
...
通常時と突進中は取り付けません。膝つきの5秒間に取り付かなければ体勢値が戻り、再び突進回避からやり直します。取り付き成功後は時間切れで強制解除されません。通常のルート登攀ではE / RBを離しても登攀は継続し、しがみつきだけを解除します。
...
右肩・頂上・背面の3か所を祓った場合だけ勝利します。地上のRecover中の反撃は体勢値だけを減らして登攀機会を作り、主の鎮静や共通Nushi進行を直接変更しません。
...
`NushiBase`、`NushiEncounterManager`、禍根・状態・スタミナの各コンポーネントと `FuchimatoiBoss` も保持しています。これらは再利用用の独立した実装であり、石走り専用の体勢処理や通常マップのボスを置き換えるものではありません。
