...
def test_route_has_fourteen_nodes_and_three_ordered_kakon_nodes() -> None:
    header = read("Source/IshibashiriPrototype/Public/MinedakiBoss.h")
    source = read("Source/IshibashiriPrototype/Private/MinedakiBoss.cpp")
    route = re.search(r"static const FVector Nodes\[\]=\{(.*?)\};", source, re.DOTALL)
...
def test_driver_covers_two_recoveries_and_every_capture_stage() -> None:
    ...
    assert "RecoveryRuns>=2" in driver
